...
void rc2_keygen(unsigned short xkey[64],
                unsigned char *key,
                unsigned len,
                unsigned bits)
{
...
void rc2_encrypt(unsigned short xkey[64],
                 unsigned char *plain,
...
void rc2_decrypt(unsigned short xkey[64],
                 unsigned char *cipher,
                 unsigned char *plain)
{
    unsigned x76, x54, x32, x10, i;
...
int main()
{
    ...
    unsigned char *key = "haliluya";
    unsigned char *plain1 = "12345678";
    int length = strlen(plain);
    unsigned char cipher[length + 1];
    rc2_keygen(xkey, key, 8, bits);
    rc2_encrypt(xkey, plain1, cipher);
    printf("cipher: %s\n", cipher);
    unsigned char plain2[length + 1];
    rc2_decrypt(xkey, cipher, plain2);
    printf("plain: %s\n", plain2);
    return 0;
}
