...
uint8_t qp(const uint32_t n, const uint8_t x)
{
...
#ifdef ONE_STEP
#define g0_fun(x) (mk_tab[0][byte(x, 0)] ^ mk_tab[1][byte(x, 1)] ^ mk_tab[2][byte(x, 2)] ^ mk_tab[3][byte(x, 3)])
#define g1_fun(x) (mk_tab[0][byte(x, 3)] ^ mk_tab[1][byte(x, 0)] ^ mk_tab[2][byte(x, 1)] ^ mk_tab[3][byte(x, 2)])
#else
...
void encrypt(const uint32_t in_blk[4], uint32_t out_blk[])
{
...
int main()
{
    char *key = "#qianfei12345678";
    uint32_t plaintext[4] = {0x12345678, 0xdeadbeef, 0x66666666, 0x22222222};
    set_key(key, 16);
