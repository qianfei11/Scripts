...
#define F(bc, x) (((S[0][(x >> 24) & 0xff] + S[1][(x >> 16) & 0xff]) ^ S[2][(x >> 8) & 0xff]) + S[3][x & 0xff])
...
void encipher(uint32_t *x)
{
	register uint32_t Xl;
	register uint32_t Xr, temp;
	short i;
...
/* x should be a 64 bit integer */
static void enblf_noswap(uint32_t *x)
{ /* Used internally */
	register uint32_t Xl;
	register uint32_t Xr, temp;
	short i;
...
void decipher(uint32_t *x)
{
	register uint32_t Xl, Xr, temp;
	register short i;
...
static short initialize_blowfish(unsigned char key[],
								 short keybytes)
{
	short i, j;
...
int generate_keys(char *k, int len)
{
	initialize_blowfish(k, len);
	return 0;
}
...
int main()
{
	...
	generate_keys((void *)keyword, 56);
	free(keyword);

	encipher((void *)ciphertext);

	for (int j = 0; j < BLOCK_SIZE; j++)
		sprintf(&((char *)cipher_tmp)[2 * j], "%.2x", ciphertext[j]);

	decipher((void *)ciphertext);
